...
// GetEnvErr returns the environment enum for the environment string passed through
func GetEnvErr(env string) (Env, error) {
	if env == "local" {
		return Local, nil
	} else if env == "internal" {
		return Internal, nil
	...
	return Local, verrors.New(verrors.InvalidArgument, "invalid env string passed in")
}

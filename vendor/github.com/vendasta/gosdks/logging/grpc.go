...
// Deprecated: We now use opencensus, which is automatically enabled. You may remove any usages of this interceptor from your application.
...
func (g *grpcInterceptor) postRequestHook(ctx context.Context, path string, responseSize int, err error, rd *requestData) {
	...
	if err != nil {
		statusCode = verrors.FromErrorWithContext(ctx, err).HTTPCode()
		logGRPCError(ctx, err)
	}

	// building fake request by parts
...
func logGRPCError(ctx context.Context, err error) {
	if err != nil {
		svcErr := verrors.FromErrorWithContext(ctx, err)
		errType := svcErr.ErrorType().String()
		if svcErr.HTTPCode() >= 500 {
			Errorf(ctx, "Error serving request: %s: %s. %s", errType, err.Error(), svcErr.GetInternalMessage())
		} else {
			Warningf(ctx, "Error serving request: %s: %s. %s", errType, err.Error(), svcErr.GetInternalMessage())
		}
	}
}
...
func fillInFromGRPCMetadata(ctx context.Context, r *requestData) {
	...
	// We can't cast directly or the header names won't be canonicalized properly
	for k, vs := range md {
		for _, v := range vs {
			httpHeaders.Add(k, v)
		}
	}
	r.HTTPRequest.Request.Header = httpHeaders

	// Add some useful context to logs
	if origin := httpHeaders.Get("Origin"); origin != "" {
		Tag(ctx, "origin", origin)
	}
	if referer := httpHeaders.Get("Referer"); referer != "" {
		Tag(ctx, "referer", referer)
	}
	if host := httpHeaders.Get("Host"); host != "" {
		Tag(ctx, "host", host)
	}
	if contentType := httpHeaders.Get("X-Content-Type"); contentType != "" {
		Tag(ctx, "content-type", contentType)
	}
	if userAgent := httpHeaders.Get("X-User-Agent"); userAgent != "" {
		Tag(ctx, "user-agent", userAgent)
	}
}
